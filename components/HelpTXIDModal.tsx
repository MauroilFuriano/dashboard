  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <FocusLock returnFocus>
        <div
          className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl p-6 relative"
          role="dialog"
          aria-modal="true"
          aria-labelledby="help-txid-modal-title"
        >

          {/* Header */}
          <div className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
            <h3 id="help-txid-modal-title" className="text-lg font-bold text-white flex items-center gap-2">
              <FileText className="text-blue-400" size={20} />
              Cos'è il TXID?
            </h3>
            <button onClick={onClose} aria-label="Chiudi modal aiuto TXID" className="text-slate-400 hover:text-white"><X size={20} /></button>
          </div>
