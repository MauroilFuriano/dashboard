  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in">
      <FocusLock returnFocus>
        <div
          className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-3xl shadow-2xl flex flex-col max-h-[85vh] sm:max-h-[90vh]"
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-800/50">
            <div>
              <h3 id="strategy-modal-title" className="text-2xl font-bold text-white">{title}</h3>
              <p className="text-emerald-400 font-medium text-sm">{subtitle}</p>
            </div>
            <button onClick={onClose} aria-label="Chiudi modal strategia" className="text-slate-400 hover:text-white transition-colors">
              <X size={24} />
            </button>
          </div>

          {/* Content Scrollable */}
          <div className="p-6 overflow-y-auto overscroll-contain space-y-8 text-slate-300 flex-1">
